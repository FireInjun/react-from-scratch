...
$configfiles = [ordered]@{
  ...
  gitignore    = @(".gitignore" , "/node_modules`n/coverage`n/dist`n.env*")
...
function dirBuilder {
  for ($i = 0; $i -lt $boilerplate.dirs.Count; $i += 1) {
    New-Item -Path . -Name $boilerplate.dirs[$i] -ItemType "directory"
  }
}
function fileBuilder {
  for ($i = 0; $i -lt $boilerplate.files.Count; $i += 1) {
    New-Item -Path $boilerplate.files[$i][2]  -ItemType "file" -Value $boilerplate.files[$i][1]
  }
}
function Configure {
  for ($i = 0; $i -lt $configfiles.Count; $i += 1) {
    New-Item -Path . -Name $configfiles[$i][0] -ItemType "file" -Value $configfiles[$i][1]
  }
}
function Installer {
  New-Item -Path . -Name $npmpack[0] -ItemType "file" -Value $npmpack[1]
